...
def next_interval(intervals):
  ...
  print(max_end_heap)
  print(max_start_heap)
  for _ in range(n):
    top_end, end_ind = heappop(max_end_heap) #last ending interval
    ...
    if -max_start_heap[0][0] >= -top_end: #latest avail start time and latest avail end time
      ...
      while max_start_heap and -max_start_heap[0][0] >= -top_end: #keep going to get the interval with a start time closest to the last end
        top_start, start_ind = heappop(max_start_heap) #any start times eliminated by this popping will never be the next interval the next time we go through the loop, becuase the next end time will be equal to or earlier than this iteration
