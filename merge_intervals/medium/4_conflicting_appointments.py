    ...
    def find_all_conflicting_appointments(self, intervals):
        ...
        for i in range(1, len(intervals)):
            ...
            if previous.end > intervals[i].start:
                ...
                prevoius = Interval(min(previous.start, intervals[i].start), max(
                    previous.end, intervals[i].end))
            else:
                previous = intervals[i]
        for conflict in conflicts:
            print(
                f"[{conflict[0].start}, {conflict[0].end}] and [{conflict[1].start}, {conflict[1].end}] conflict")
        return conflicts
...
def main():
    ...
    sol.find_all_conflicting_appointments([Interval(4, 5), Interval(
        2, 3), Interval(3, 6), Interval(5, 7), Interval(7, 8)])
